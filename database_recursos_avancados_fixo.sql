-- =====================================================
-- RECURSOS AVANÇADOS - ATUALIZAÇÃO DO BANCO (VERSÃO CORRIGIDA)
-- Upload, Aprovações, Metas, Relatórios
-- =====================================================

-- IMPORTANTE: Execute este script no phpMyAdmin
-- Se alguma coluna já existir, comente a linha correspondente

-- 1. ADICIONAR CAMPOS PARA WORKFLOW DE APROVAÇÃO
-- Remova os comentários (--) das linhas que você NÃO executou ainda
...
-- 8. CRIAR DIRETÓRIO PARA ANEXOS
-- IMPORTANTE: Você precisa criar a pasta manualmente:
-- 1. Crie a pasta: uploads/anexos
-- 2. Dê permissão 755 (via FTP ou gerenciador de arquivos)
...
-- Inserir meta de exemplo para cada departamento
...
-- Verificar novas colunas na tabela contas_pagar
SHOW COLUMNS FROM contas_pagar LIKE 'aprovacao%';
SHOW COLUMNS FROM contas_pagar LIKE 'aprovado_por';
SHOW COLUMNS FROM contas_pagar LIKE 'data_aprovacao';
SHOW COLUMNS FROM contas_pagar LIKE 'motivo_rejeicao';
