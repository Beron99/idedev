...
-- 2. ADICIONAR COLUNAS NA TABELA USUARIOS (execute linha por linha se der erro)
ALTER TABLE usuarios ADD COLUMN role ENUM('admin', 'gestor', 'usuario') DEFAULT 'usuario' AFTER senha;
ALTER TABLE usuarios ADD COLUMN departamento_id INT AFTER role;
ALTER TABLE usuarios ADD COLUMN ativo BOOLEAN DEFAULT TRUE AFTER departamento_id;
ALTER TABLE usuarios ADD COLUMN foto_perfil VARCHAR(255) AFTER ativo;
ALTER TABLE usuarios ADD COLUMN telefone VARCHAR(20) AFTER foto_perfil;
ALTER TABLE usuarios ADD COLUMN ultimo_acesso TIMESTAMP NULL AFTER telefone;

-- 3. ADICIONAR ÍNDICES NA TABELA USUARIOS
ALTER TABLE usuarios ADD INDEX idx_role (role);
ALTER TABLE usuarios ADD INDEX idx_departamento (departamento_id);
...
ALTER TABLE contas_pagar ADD COLUMN departamento_id INT AFTER usuario_id;
ALTER TABLE contas_pagar ADD INDEX idx_departamento (departamento_id);
...
ALTER TABLE contas_pagar ADD COLUMN prioridade ENUM('baixa', 'media', 'alta', 'urgente') DEFAULT 'media' AFTER status;
ALTER TABLE contas_pagar ADD COLUMN posicao_kanban INT DEFAULT 0 AFTER prioridade;
ALTER TABLE contas_pagar ADD INDEX idx_prioridade (prioridade);
...
-- Adicionar coluna gestor_nome na tabela departamentos (para facilitar consultas)
ALTER TABLE departamentos ADD COLUMN gestor_nome VARCHAR(100) AFTER responsavel_id;
